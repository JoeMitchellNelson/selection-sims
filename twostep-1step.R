...
database$meanw <- mean(database$w1)

database$w1 <- database$m + rep(rnorm(n,mean=0,sd=1),each=2)
...
#   interNDraws    = 500,
#   interUnifDraws = c(),
#   interNormDraws = c("draws_w","draws_x"),
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['alt1']] = a0 + b_w*w1
  V[['alt2']] = b_w*w2
  V[['alt3']] = b_x*x3  + b_cost*cost3 + b_int*((b_w*w1 - b_w*meanw))*x3
  V[['alt4']] = b_x*x4  + b_cost*cost4 + b_int*((b_w*w1 - b_w*meanw))*x4
