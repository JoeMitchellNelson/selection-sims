...
function (apollo_beta, apollo_inputs, functionality = "estimate") 
{
  ...
  P = list()
  V = list()
  V[["alt1"]] = (random_statquo * statusquo_1 + random_ben * 
                   ben_1 + random_cost * cost_1 + beta_imr * apollo_inputs$apollo_scaling["beta_imr"] * 
                   imr((random_alpha + alpha_spring * apollo_inputs$apollo_scaling["alpha_spring"] * 
                          spring + alpha_factor1 * apollo_inputs$apollo_scaling["alpha_factor1"] * 
                          factor1 + alpha_hav_ZIPSCORES * apollo_inputs$apollo_scaling["alpha_hav_ZIPSCORES"] * 
                          hav_ZIPSCORES)/sqrt((a1 * apollo_inputs$apollo_scaling["a1"])^2 + 
                                                (pi^2/3))))/rescaler(c(a1 * apollo_inputs$apollo_scaling["a1"], 
                                                                       b1 * apollo_inputs$apollo_scaling["b1"], b2 * apollo_inputs$apollo_scaling["b2"], 
                                                                       c1 * apollo_inputs$apollo_scaling["c1"], c2 * apollo_inputs$apollo_scaling["c2"], 
                                                                       c3 * apollo_inputs$apollo_scaling["c3"], d1 * apollo_inputs$apollo_scaling["d1"], 
                                                                       d2 * apollo_inputs$apollo_scaling["d2"], d3 * apollo_inputs$apollo_scaling["d3"], 
                                                                       d4 * apollo_inputs$apollo_scaling["d4"]), c((statusquo_1 + 
                                                                                                                      statusquo_2)/2, (ben_1 + ben_2)/2, (cost_1 + cost_2)/2), 
                                                                     n = nObs)
  V[["alt2"]] = (random_statquo * statusquo_2 + random_ben * 
                   ben_2 + random_cost * cost_2 + beta_imr * apollo_inputs$apollo_scaling["beta_imr"] * 
                   imr((random_alpha + alpha_spring * apollo_inputs$apollo_scaling["alpha_spring"] * 
                          spring + alpha_factor1 * apollo_inputs$apollo_scaling["alpha_factor1"] * 
                          factor1 + alpha_hav_ZIPSCORES * apollo_inputs$apollo_scaling["alpha_hav_ZIPSCORES"] * 
                          hav_ZIPSCORES)/sqrt((a1 * apollo_inputs$apollo_scaling["a1"])^2 + 
                                                (pi^2/3))))/rescaler(c(a1 * apollo_inputs$apollo_scaling["a1"], 
                                                                       ...
                                                                       d4 * apollo_inputs$apollo_scaling["d4"]), c((statusquo_1 + 
                                                                                                                      statusquo_2)/2, (ben_1 + ben_2)/2, (cost_1 + cost_2)/2), 
                                                                     n = nObs)
  V[["alt3"]] = random_ben * ben_3 + random_cost * cost_3 + 
    random_statquo * statusquo_3
  V[["alt4"]] = random_alpha + alpha_spring * apollo_inputs$apollo_scaling["alpha_spring"] * 
    spring + alpha_factor1 * apollo_inputs$apollo_scaling["alpha_factor1"] * 
    factor1 + alpha_hav_ZIPSCORES * apollo_inputs$apollo_scaling["alpha_hav_ZIPSCORES"] * 
    hav_ZIPSCORES
  V[["alt5"]] = 0
  mnl_settings = list(alternatives = c(alt1 = 1, alt2 = 2, 
                                       alt3 = 3, alt4 = 4, alt5 = 5), avail = list(alt1 = av1, 
                                                                                   alt2 = av2, alt3 = av3, alt4 = av4, alt5 = av5), choiceVar = choice, 
                      V = lapply(V, "*", scale_RP), rows = (av4 == 1 | 
                                                              av5 == 1), componentName = "MNL-RP")
  P[["RP"]] = apollo_mnl(c(mnl_settings, componentName2 = "RP"), 
                         functionality)
  mnl_settings$V = lapply(V, "*", scale_SP)
  mnl_settings$rows = (av1 == 1 | av2 == 1 | av3 == 1)
  mnl_settings$componentName = "MNL-SP"
  P[["SP"]] = apollo_mnl(c(mnl_settings, componentName2 = "SP"), 
                         functionality)
...
function (bVar, countIter = FALSE, writeIter = FALSE, sumLL = FALSE, 
          getNIter = FALSE) 
{
  if (getNIter) 
    return(nIter)
  b <- c(bVar, bFixedVal)
  ll <- parallel::clusterCall(cl = cl, fun = apollo_parProb, 
                              b = b)
  ll <- unlist(ll)
  if (!workInLogs) 
    ll <- log(ll)
  ...
  if (analyticGrad) {
    ...
    if (countIter && newIter) 
      nIter <<- nIter + 1
    if (writeIter && newIter) 
      apollo_writeTheta(bVar, sumll, modelName)
    if (newIter) 
      preLL <<- sumll
  }
  else if (estAlg == "bfgs" && exists("lastFuncParam", 
                                      envir = globalenv())) {
    lastFuncParam <- get("lastFuncParam", envir = globalenv())
    newIter <- !anyNA(lastFuncParam) && length(lastFuncParam) == 
      length(bVar) && all(lastFuncParam == bVar)
    if (countIter && newIter) 
      nIter <<- nIter + 1
    if (writeIter && newIter) 
      apollo_writeTheta(bVar, sumll, modelName)
