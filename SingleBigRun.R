...
model = apollo_estimate(apollo_beta, apollo_fixed,
                        ...
                        estimate_settings=list(hessianRoutine="numDeriv",silent=F))
...
#######################################################
#######################################################
###### UNCORRECTED MIXED LOGIT ########################
#######################################################
#######################################################

database2 <- database
database <- database %>% dplyr::filter(SP==1)
...
### Set core controls
apollo_control = list(
  modelName ="Uncorrected mixed logit",
  modelDescr ="Mixed logit model",
  indivID   ="ID",  
  mixing    = TRUE, 
  nCores    = 12
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  ### List of utilities: these must use the same names as in mnl_settings, order is irrelevant
  V = list()
  V[['alt3']] = scale*(b_x*x3  + b_cost*cost3)
  V[['alt4']] = scale*(b_x*x4  + b_cost*cost4)
  
  
  mnl_settings = list(
    alternatives  = c(alt3=3, alt4=4),
    avail         = list(alt3=av_3,alt4=av_4),
    choiceVar     = choice,
    V             = V
  )
...
#saveRDS(model2,"~/selection-sims/bigsim/correctedmixl.Rds")
#saveRDS(model,"~/selection-sims/bigsim/uncorrectedmixl.Rds")
#saveRDS(db,"~/selection-sims/bigsim/simfulldata.Rds")
