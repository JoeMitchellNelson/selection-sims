...
for (i in 1:100) {
  ...
  # imr function, to be called in alternative-specific utility functions during estimation
  imr <- function (x) {dnorm(-1*x)/(1-pnorm(-1*x))}
  
  # not necessary to set a seed here, but I needed this code when I was running many small simulations inside a function
  ...
  stupidmatrix <- function (a1,b1,b2,c1,c2,c3,d1,d2,d3,d4,statquo,att,cost) {
    
    sqrt(
      statquo*(statquo*(b1^2+b2^2) + att*(b1*c1+b2*c2) + cost*(b1*d1+b2*d2)) +
        att*(statquo*(b1*c1+b2*c2) + att*(c1^2+c2^2+c3^2) + cost*(c1*d1+c2*d2+c3*d3)) +
        cost*(statquo*(b1*d1+b2*d2) + att*(c1*d1+c2*d2+c3*d3) + cost*(d1^2+d2^2+d3^2+d4^2)) +
        (pi^2)/3
    )
  }
  
  #apollo_beta <- startpars2
  ...
  apollo_probabilities <<- function(apollo_beta, apollo_inputs, functionality="estimate"){
    ...
    V[['alt4']] = (random_cost*cost1 + random_att*att1 + random_statquo*statquo +
                     beta_imr*imr((alpha_iv*iv + mu_alpha)/sqrt(a1^2+(pi^2/3))))/
      stupidmatrix(a1,b1,b2,c1,c2,c3,d1,d2,d3,d4,statquo,(att1+att2)/2,(cost1+cost2)/2)
    V[['alt5']] = (random_cost*cost2 + random_att*att2 + random_statquo*statquo +
                     beta_imr*imr((alpha_iv*iv + mu_alpha)/sqrt(a1^2+(pi^2/3))))/
      stupidmatrix(a1,b1,b2,c1,c2,c3,d1,d2,d3,d4,statquo,(att1+att2)/2,(cost1+cost2)/2)
...
#saveRDS(results,"~/selection-sims/100runsof2k.rds")
